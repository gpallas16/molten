...
QtObject {
    ...
    function wifiIcon(strength, enabled, connected) {
        if (!enabled) return wifiOff
        if (!connected) return wifiX
        if (strength >= 75) return wifiHigh
        if (strength >= 50) return wifiMedium
        if (strength >= 25) return wifiLow
        return wifiNone
    }

    /** Get volume icon based on level (0-1) and mute state */
    function volumeIcon(volume, muted) {
        if (muted || volume === 0) return speakerX
        if (volume < 0.33) return speakerLow
        if (volume < 0.66) return speakerNone
        return speakerHigh
    }
    ...
    function batteryIcon(percentage, charging) {
        ...
        if (percentage >= 60) return batteryHigh
